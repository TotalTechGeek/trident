...
| $root     | Use with $out to merge items from this to the root of the template. The main purpose is to allow you to use properties like `$meta` without them being erased as directives, since all dollar sign properties at the top level are stripped from the template | boolean                                 | $out                         |
